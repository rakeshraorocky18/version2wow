...
export class EventsController {
  ...
  @ApiOperation({ summary: 'Dashboard summary across all my events' })
  async getDashboard(@Req() req: any) {
    return this.eventsService.getDashboard(req.user.id);
  }
  ...
  @Post(':id/invitations')
  @ApiOperation({ summary: 'Create invitations for guests (with RSVP links)' })
  async createInvitations(@Param('id') id: string, @Req() req: any, @Body() dto: CreateInvitationsDto) {
    return this.eventsService.createInvitations(id, req.user.id, dto);
  }

  @Get(':id/invitations')
  @ApiOperation({ summary: 'List invitations for an event' })
  async getInvitations(@Param('id') id: string, @Query('status') status?: InvitationStatus) {
    return this.eventsService.getInvitations(id, status);
  }

  @Post('invitations/send')
  @ApiOperation({ summary: 'Send invitations via their channel (WhatsApp/SMS/Email)' })
  async dispatchInvitations(@Req() req: any, @Body() dto: SendInvitationsDto) {
    return this.eventsService.sendInvitations(req.user.id, dto);
  }

  // ─── RSVPs ───

  @Get(':id/rsvps')
  @ApiOperation({ summary: 'List RSVP responses for an event' })
  async getRsvps(@Param('id') id: string) {
    return this.eventsService.getRsvps(id);
  }
}

// Guest-facing RSVP endpoints — no auth, accessed via the unique invitation token
@ApiTags('rsvp')
@Controller('rsvp')
export class PublicRsvpController {
  ...
  @Get(':token')
  @ApiOperation({ summary: 'View invitation + event details (RSVP response page)' })
  async getInvitation(@Param('token') token: string) {
    return this.eventsService.getInvitationByToken(token);
  }

  @Post(':token')
  @ApiOperation({ summary: 'Submit RSVP response' })
  async submitRsvp(@Param('token') token: string, @Body() dto: PublicRsvpDto) {
    return this.eventsService.submitRsvpByToken(token, dto);
  }
}
